...
var ctf_methods = function() {
      ...
      this.step = function() {
         ...
         if (ctf_var.flagrunners.length != 0) {
            ...
            //hamsters.run(ctf, ctf.ctf_methods().moveFlagRunners(), ctf.ctf_methods().test2(), 1, true);
         }
         if (ctf_var.attackers.length != 0) {
            ctf_methods().moveAttackers()
            //hamsters.run(ctf, ctf.ctf_methods().moveAttackers(), ctf.ctf_methods().test2(), 1, true);
         }
         if (ctf_var.eflagrunners.length != 0) {
            ctf_methods().moveEFlagRunners()
            //hamsters.run(ctf, ctf.ctf_methods().moveEFlagRunners(), ctf.ctf_methods().test2(), 1, true);
         }
         if (ctf_var.eattackers.length != 0) {
            ctf_methods().moveEAttackers()
            //hamsters.run(ctf, ctf.ctf_methods().moveEAttackers(), ctf.ctf_methods().test2(), 1, true);
         }
