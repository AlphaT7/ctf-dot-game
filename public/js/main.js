...
var main = {
    ...
    methods: {
        ...
        drawPlayerBoundry: function() {
            ...
            mv.ctx.beginPath();
            mv.ctx.rect(mv.dropboundry.x, mv.dropboundry.y, mv.dropboundry.w, mv.dropboundry.h);

            // check if we hover it, fill red, if not make it transparent
            mv.ctx.fillStyle = mv.ctx.isPointInPath(mv.mouse.x, mv.mouse.y) ? mv.colors.st_red : mv.colors.clear;
            mv.ctx.fill();
            mv.ctx.restore();
        },
        ...
        moveFlagRunners: function() {
            ...
            for (var dot in mvd) {
                if (mvd[dot].dottype == 'flagrunner' && mvd[dot].live == true) {
                    ...
                    var targetaquired = x == mv.flags[0].x && y == mv.flags[0].y;
                    var targetoffset = x - 7 == mv.flags[0].x && y - 7 == mv.flags[0].y;
                    ...
                    var testX = target.x - (mvd[dot].x + velX);
                    var testY = target.y - (mvd[dot].y + velY);
        ...
        */
        ...
        step: function() {
            ...
            if ( mg.length > 200 ) { mg.splice(0,1); }
            ...
            for (let dot in mved) {
                if (mved[dot].dottype == 'attacker' && mved[dot].live == true) {
                    mm.drawCircle(mved[dot].x, mved[dot].y, mved[dot].r, mv.colors.orange);
                }
                else if (mved[dot].dottype == 'flagrunner' && mved[dot].live == true) {
                    mm.drawCircle(mved[dot].x, mved[dot].y, mved[dot].r, mv.colors.orange);
                }
            }
...
socket.on('server2client', function() {
    ...
    let mv = main.variables;
    $('#latency').html('Latency: ' + (Date.now() - main.variables.latencyarray[mv.latencycount]) + ' ms');
    mv.latencycount++;
...
socket.on('gamestatus', function(data) {
    ...
    if (data.stepcounter > (main.gamestate[main.gamestate.length-1].stepcounter + 100)) {
        ...
        if (mv.host == true) {
            ...
            mv.flags = JSON.parse(JSON.stringify(data.flags));
            ...
            mv.dots = JSON.parse(JSON.stringify(data.dots2));
            ...
            // for the non-host machine, must inverse flags to make the display correctly
